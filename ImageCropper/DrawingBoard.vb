...
Friend Class DrawingBoard
    ...
    'Member Variables
    Private m_MouseButtons As System.Windows.Forms.MouseButtons = System.Windows.Forms.MouseButtons.Left
...
#Region "Public Properties"
    ...
    Public Property ZoomFactor() As Double
        ...
        Set(ByVal value As Double)
            ...
            End If
            Me.Refresh()
            RaiseEvent ZoomFactorChanged(m_ZoomFactor)
        End Set
    End Property
    ...
    Dim m_AutoZoom As Boolean = True
...
#End Region
    ...
    Private Sub DrawImage(ByRef g As Graphics)
        ...
        Else
            Dim intOriginX As Integer = m_Origin.X
            Dim intOriginY As Integer = m_Origin.Y
            If m_DrawWidth > m_OriginalImage.Width Then
                intOriginX = CInt((m_DrawWidth - m_OriginalImage.Width) / 2) * -1
            End If
            If m_DrawHeight > m_OriginalImage.Height Then
                intOriginY = CInt((m_DrawHeight - m_OriginalImage.Height) / 2) * -1
            End If
            SrcRect = New System.Drawing.Rectangle(intOriginX, intOriginY, m_DrawWidth, m_DrawHeight)
        End If
    ...
    Private Sub ImageViewer_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseDown
        ...
        Dim ptrCur As IntPtr = My.Resources.CloseHandCursor.GetHicon
        Dim newCursor As New Cursor(ptrCur)
        Me.Cursor = newCursor
    ...
    Private Sub DrawingBoard_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseUp
        ...
        Dim ptrCur As IntPtr = My.Resources.OpenHandCursor.GetHicon
        Dim newCursor As New Cursor(ptrCur)
        Me.Cursor = newCursor
        ...
        If Not PanMode Then
            ...
            Else
                Dim NewOrigin As Point
                NewOrigin = New Point(CInt(Me.Origin.X + (Selected_Rectangle.X / ZoomFactor)),
                                              Me.Origin.Y + (Selected_Rectangle.Y / ZoomFactor))

                Dim oCropImage As New Bitmap(CInt(Selected_Rectangle.Width / ZoomFactor), CInt(Selected_Rectangle.Height / ZoomFactor))

                Dim g = Graphics.FromImage(oCropImage)
                Dim destRect As New Rectangle(-NewOrigin.X - If(SrcRect.X < 0, SrcRect.X, 0), -NewOrigin.Y - If(SrcRect.Y < 0, SrcRect.Y, 0), m_OriginalImage.Width, m_OriginalImage.Height)

                Dim sourceRect As New Rectangle(0, 0, m_OriginalImage.Width, m_OriginalImage.Height)
                g.DrawImage(m_OriginalImage, destRect, sourceRect, GraphicsUnit.Pixel) ' 0, 0, CInt(Selected_Rectangle.Width * NewFactor), CInt(Selected_Rectangle.Height * NewFactor))
                g_Sequence += 1
                oCropImage.Save(GetWorkingDirectory() & g_Sequence.ToString & ".jpg", ImageFormat.Jpeg)
                oCropImage.Dispose()
            End If
        End If
    End Sub
    ...
    Private Sub DrawingBoard_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        ...
        If Me.StretchImageToFit Then Me.Refresh()
    End Sub
